...
| 6      | Criteria not reconciled to overall          | **Fixed** — when the criteria additively partition the paper (their maxMarks sum to `totalMarks`), the overall mark is reconciled to the sum of the awarded criterion marks so the score placard cannot contradict the breakdown.                                    |
...
| 12     | Improvement target ignored the tier ceiling | **Fixed** — the improve-answer target band and the "Improved Response" exemplar band/regenerate control are now capped at the tier ceiling (`getBandForMark`), and the saved exemplar's mark is chosen via the new `markForBand` inverse so its mark and band agree. |
| 13, 16 | Remaining lower-priority items              | **Open** — unknown-verb fallback-tier ceiling (largely theoretical given the Tier-2 fallback), half-marks.                                                                                                                                                           |
