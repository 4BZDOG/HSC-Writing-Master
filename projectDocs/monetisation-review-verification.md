...
claim (Stripe _throws_ specifically when `automatic_tax` is enabled with no
...
- **Claim 2 (origin-address throw) — needs a human/operator confirmation.** Official
  ...
  session creation _throws_ on a missing account origin address (Stripe usually
