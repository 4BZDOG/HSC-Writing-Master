...
*   **Math/science notation**: `^` for superscript, `_` for subscript, `\sqrt{}`, `\frac{a}{b}`, `\vec{F}`,
    ...
    `utils/mathNotation.ts`) but the content between them is passed through unchanged, so `$ax$` still
    renders as flat text "ax" rather than a real subscripted "a"+"x" — write `a_x` directly instead.