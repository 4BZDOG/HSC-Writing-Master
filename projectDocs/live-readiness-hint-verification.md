...
**The one real issue:** the level-3 readiness label is `"Developing"`, which is *identical*
...
| 8 | Tier identity preserved (base hue not morphed by readiness) | **CONFIRMED** | `chroma.background`/`accent` derive from `verbTier` only (`Editor.tsx:295-309`); `readinessAccent` is a separate memo adding only `hex`+`glow` (`:327-331`). Readiness feeds the veil *lift* (dark overlay opacity) but never the base tier colour. |
