    ...
    const processRecursively = (segment: string | React.ReactNode): React.ReactNode[] => {
            ...
            if (parts.length > 1) {
                return parts.map((part, i) => {
                    if (part.startsWith('**') && part.endsWith('**')) {
                        return React.createElement('strong', { key: i, className: "font-bold text-white light:text-slate-900 print:text-[rgb(var(--color-text-primary))]" }, processRecursively(part.slice(2, -2)));
                    }
            ...
            if (parts.length > 1) {
                return parts.map((part, i) => {
                    if (part.startsWith('*') && part.endsWith('*') && part.length > 2) {
                         return React.createElement('em', { key: i, className: "italic text-white/90 light:text-slate-800 print:text-[rgb(var(--color-text-secondary))]" }, processRecursively(part.slice(1, -1)));
                    }
                    return processRecursively(part);
                }).flat();
            }
        ...
        if (segment.match(REGEX_SUPERSCRIPT)) {
             ...
             if (parts.length > 1) {
                 return parts.map((part, i) => {
                     if (part.startsWith('^')) {
                         return React.createElement('sup', { key: i, className: "text-[0.7em]" }, part.slice(1));
                     }
                     return processRecursively(part);
                 }).flat();
             }
             ...
             if (parts.length > 1) {
                 return parts.map((part, i) => {
                     if (part.startsWith('_')) {
                         return React.createElement('sub', { key: i, className: "text-[0.7em]" }, part.slice(1));
                     }
                     return processRecursively(part);
                 }).flat();
             }
        ...
        if (verbRegex || keywordRegex) {
            ...
            if (verbRegex) {
                nodes = nodes.flatMap(n => {
                    ...
                    return parts.map((part, i) => {
                        if (verbRegex.test(part)) { // Capturing group matched
                             return React.createElement('span', { key: `v-${i}`, className: "font-black text-[rgb(var(--color-accent))]" }, part);
                        }
                        return part;
                    });
                });
                ...
                nodes = nodes.flatMap(n => {
                    ...
                    return parts.map((part, i) => {
                        if (keywordRegex.test(part)) {
                             return React.createElement('span', { key: `k-${i}`, className: "font-bold text-emerald-400 light:text-emerald-700" }, part);
                        }
                        return part;
                    });
                });
...
export const renderEditorHighlights = (text: string, keywords?: string[], verb?: PromptVerb): React.ReactNode => {
    if (!text) return null;
            ...
            nodes = nodes.flatMap(n => {
                ...
                return parts.map((part, i) => {
                    if (verbRegex.test(part)) {
                        ...
                        return React.createElement('span', { key: `v-${i}`, className: "bg-[rgb(var(--color-accent))]/20 text-[rgb(var(--color-accent))]" }, part);
                    }
                    return part;
                });
            });
        ...
        if (keywordRegex) {
            nodes = nodes.flatMap(n => {
                ...
                return parts.map((part, i) => {
                    if (keywordRegex.test(part)) {
                        // Removed padding px-0.5 to prevent horizontal drift/ghosting
                        return React.createElement('span', { key: `k-${i}`, className: "bg-emerald-500/20 text-emerald-400" }, part);
                    }
                    return part;
                });
            });
