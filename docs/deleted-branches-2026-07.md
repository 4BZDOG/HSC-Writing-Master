# Stale branch manifest — recorded 2026-07-25, still pending

**Status: the deletion has NOT been run.** This file was written ahead of it,
in the past tense, but ref deletion returns 403 through the Claude Code web
sandbox's git proxy — so it has to be run from a local clone or the GitHub UI
(see the bottom of this file), and as of 2026-07-27 all of these branches are
still on the remote. The list below is the record to check against when the
deletion does happen; nothing here is out of date except the tense it was
originally written in.

Every branch listed is either fully merged into `main` or judged too stale to
...
Nine of these are true ancestors of `main` and can be confirmed at any time
with `git branch -r --merged origin/main`. The rest were squash-merged, so
their commits are not ancestors even though their content landed — which is
why each row names the PR that carried it. That distinction is the whole
reason this file exists: the "ahead by N" counts on the branches dashboard
are squash artefacts, not unlanded work.
...
| `claude/sample-answer-length-modal-y4o9t3`            | `efcbcf2c79e7130050c2434c0ce12047740ea9ea` | merged into `main` (ancestor)                    |
...
| `claude/user-agreements-onboarding-tynhpl`            | `9ba706f12943f8fcc8fb4828e4c69149c692c206` | PRs #110, #111 merged (ancestor)                 |
...
| `claude/active-branches-review-vaxdm9`                | `60c70683f0ae7446b23d0436333344f5cfd8ff06` | PR #107 merged — the branch this file arrived on |
| `claude/writing-prompt-ui-improvements-ic6ewn`        | see PR #114                                | PRs #112, #113, #114 merged                      |
...
`main`, and any branch with an open pull request at the time of deletion. The
command below matches `^claude/` and would take those too — check the open PR
list first.
