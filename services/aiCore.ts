...
export class ApiGuard {
  ...
  private cleanupOldErrors() {
    ...
    this.errors = this.errors.filter(
      (error) => now - error.timestamp < TIME_WINDOW_MS
    );
...
class ApiMonitor {
    ...
    constructor() {
        const storedTotals = safeGetItem<{ calls: number, tokens: number }>(STORAGE_KEYS.API_STATS, { calls: 0, tokens: 0 });
        this.status = {
            sessionCalls: 0,
            sessionTokens: 0,
...
const callGeminiWithRetry = async <T>(apiCall: () => Promise<T>, maxRetries: number = MAX_RETRIES): Promise<T> => {
  if (apiGuard.isBlocked()) {
    throw new ApiKeyError(apiGuard.getStatus().blockReason || "API calls are temporarily blocked due to high error rates. Please wait a moment.");
  }
  ...
  while (true) {
    try {
      ...
      if (errorMsg.includes('API key not valid') || status === 403 || errorMsg.includes('PERMISSION_DENIED')) {
        throw new ApiKeyError("Access Denied: Your API Key is invalid, expired, or lacks permission. Please check your settings.");
      }
      ...
      if (status === 404 || errorMsg.includes('NOT_FOUND')) {
         throw new Error("The requested AI model is currently unavailable or deprecated. Please check configuration.");
      }
      ...
      if (attempt < maxRetries) {
           ...
           console.warn(`[API Retry] ${reason} (${errorMsg}). Retrying in ${Math.round(delay)}ms... (Attempt ${attempt}/${maxRetries})`);
      ...
      if (isQuotaError) {
          throw new QuotaExceededError("Usage Limit Reached: You have exceeded the API quota. Please check your Google AI Studio billing or try again later.");
      }
...
export const generateContentWithRetry = async (request: any): Promise<GenerateContentResponse> => {
    if (!process.env.API_KEY) {
        throw new ApiKeyError("API Key is missing. Please configure your API Key via the selection dialog.");
    }

    const client = new GoogleGenAI({ apiKey: process.env.API_KEY });
    
    const response = await callGeminiWithRetry<GenerateContentResponse>(() => client.models.generateContent(request));
    ...
    if (response?.candidates && response.candidates.length > 0) {
        ...
        if (candidate.finishReason && candidate.finishReason !== 'STOP') {
             ...
             if (candidate.finishReason === 'SAFETY') {
                 throw new Error("The AI response was blocked due to safety settings. Please modify your prompt and try again.");
             }
