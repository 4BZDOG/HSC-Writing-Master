    ...
    if (step === 'preview' && !previewInitialisedRef.current) {
      ...
      importedCourses.forEach((imp) => {
        const match = existingCourses.find(
          (ex) => ex.name.trim().toLowerCase() === imp.name.trim().toLowerCase()
        );
        if (match) {
          ...
          <div className="w-full max-w-2xl space-y-8">
            <div className="text-center space-y-2">
              ...
              <p className="text-[rgb(var(--color-text-secondary))]">
                Upload a JSON file containing Courses or Topics.
              </p>
            </div>
            ...
            <div className="bg-[rgb(var(--color-bg-surface))]/50 border border-[rgb(var(--color-border-secondary))] rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-center sm:text-left">
                ...
                <p className="text-xs text-[rgb(var(--color-text-muted))] mt-1">
                  Use this schema to generate compatible content with LLMs.
                </p>
              </div>
              <button
          ...
          <div className="flex-1 overflow-y-auto md:overflow-hidden grid grid-cols-1 md:grid-cols-2 custom-scrollbar">
            ...
            <div className="flex flex-col md:h-full border-b md:border-b-0 md:border-r border-[rgb(var(--color-border-secondary))]">
              <div className="px-4 py-3 bg-[rgb(var(--color-bg-surface-elevated))]/50 border-b border-[rgb(var(--color-border-secondary))] flex items-center justify-between">
                <h4 className="t-section text-[rgb(var(--color-text-muted))]">Select Content</h4>
                <div className="flex gap-2">
                  <button
                    onClick={selectAll}
                    className="text-[10px] font-bold text-[rgb(var(--color-accent))] hover:text-[rgb(var(--color-accent-glow))]"
                  >
                    All
                  </button>
                  ...
                  <button
                    onClick={deselectAll}
                    className="text-[10px] font-bold text-[rgb(var(--color-text-muted))] hover:text-[rgb(var(--color-text-secondary))]"
                  >
                    None
                  </button>
                </div>
            ...
            <div className="flex flex-col md:h-full md:overflow-y-auto custom-scrollbar bg-[rgb(var(--color-bg-surface))]/30">
              <div className="px-4 py-3 bg-[rgb(var(--color-bg-surface-elevated))]/50 border-b border-[rgb(var(--color-border-secondary))]">
                <h4 className="t-section text-[rgb(var(--color-text-muted))]">Configuration</h4>
              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {importedCourses.map((course) => {
                      ...
                      return (
                        <div
                          key={course.id}
                          className="p-3 rounded-lg bg-[rgb(var(--color-bg-surface))] border border-[rgb(var(--color-border-secondary))]"
                        >
                          <div
                            className="text-xs font-medium text-[rgb(var(--color-text-secondary))] mb-2 truncate"
                            title={course.name}
                          >
                            {course.name}
                          </div>
          ...
          <div className="p-8 space-y-6 max-w-2xl mx-auto w-full overflow-y-auto custom-scrollbar">
            <div className="text-center mb-4">
              ...
              <p className="text-[rgb(var(--color-text-secondary))] text-sm">
                Select where to add "{importedTopic.name}"
              </p>
            </div>
