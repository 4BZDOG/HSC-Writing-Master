...
export const ModalHeader: React.FC<{ title: string; subtitle: string, icon?: React.ReactNode }> = ({ title, subtitle, icon }) => (
...
export const ActionButtons: React.FC<{ onCancel: () => void; onConfirm: () => void; confirmText: string; isConfirmDisabled?: boolean }> = ({ onCancel, onConfirm, confirmText, isConfirmDisabled = false }) => (
  <div className="px-6 py-4 bg-[rgb(var(--color-bg-surface-inset))]/50 border-t border-[rgb(var(--color-border-secondary))] flex justify-end space-x-3 flex-shrink-0">
    <button onClick={onCancel} className="py-2 px-4 rounded-lg text-sm font-semibold text-[rgb(var(--color-text-muted))] bg-[rgb(var(--color-bg-surface-light))] hover:bg-[rgb(var(--color-border-secondary))] transition">
      Cancel
    </button>
    <button 
      onClick={onConfirm} 
