  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleClose}>
      ...
      >
        <div className="relative px-6 py-5 border-b border-[rgb(var(--color-border-secondary))] light:border-slate-200 bg-[rgb(var(--color-bg-surface))] light:bg-slate-50/50">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-[rgb(var(--color-text-primary))] light:text-slate-900">Create New Course</h2>
                <p className="text-sm text-[rgb(var(--color-text-muted))] light:text-slate-500">Define a course and its syllabus outcomes.</p>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col overflow-hidden bg-[rgb(var(--color-bg-surface))] light:bg-white">
          <div className="p-6 space-y-6 flex-grow overflow-y-auto">
            <div>
              <label htmlFor="course-name" className="block text-sm font-medium text-[rgb(var(--color-text-secondary))] light:text-slate-700 mb-2">
                Course Name
              </label>
              <input
                type="text"
            ...
            <div>
              ...
              <div className="space-y-3">
                 {outcomes.map((outcome, index) => (
                    <div key={index} className="flex items-start space-x-2 bg-[rgb(var(--color-bg-surface-inset))]/50 light:bg-slate-50 p-3 rounded-lg border border-[rgb(var(--color-border-secondary))] light:border-slate-200">
                        <input
                          type="text"
                          value={outcome.code}
          ...
          {error && <p className="px-6 text-red-400 light:text-red-600 text-sm bg-red-900/30 light:bg-red-50 p-3 rounded-md mx-6 border border-red-500/20 light:border-red-200">{error}</p>}
