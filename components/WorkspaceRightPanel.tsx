...
const WorkspaceRightPanel: React.FC<WorkspaceRightPanelProps> = ({
  ...
  const progressScore = useMemo(() => {
    ...
    if (keywords.length > 0) {
        const used = keywords.filter(kw => {
            ...
            return variants.some(v => new RegExp(`\\b${escapeRegExp(v)}\\b`, 'i').test(debouncedUserAnswer.toLowerCase()));
        });
  ...
  const buttonConfig = useMemo(() => {
      ...
      let predictedBand = 1;
      if (progressScore >= 0.90) predictedBand = 6;
  ...
  return (
    <div className={`${isFocusMode ? 'col-span-1 max-w-5xl mx-auto w-full' : 'lg:col-span-7'} flex flex-col gap-6 h-full pt-0 pb-20`}>
        
        <div className="relative group">
            <div 
                className="flex flex-col relative transition-all duration-700 shadow-2xl rounded-[32px]"
            >
                ...
                <div className="absolute bottom-12 right-12 z-20">
                    ...
                    >
                        {isEvaluating ? (
                            ...
                            <>
                                <Sparkles className={`w-6 h-6 ${progressScore > 0.85 ? 'text-white/90 animate-pulse' : 'text-white/70'}`} />
        ...
        <div id="evaluation-results" className="scroll-mt-24">
            {evaluationError && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-8 rounded-[40px] animate-fade-in flex items-start gap-5 shadow-2xl shadow-red-900/10 backdrop-blur-xl">
                    ...
                    <div>
                        <h4 className="font-black uppercase tracking-[0.2em] text-xs mb-2">System Interruption</h4>
            ...
            {userRole === 'admin' && !evaluationResult && (
                 <div className="mt-12 p-6 rounded-[32px] bg-[rgb(var(--color-bg-surface-inset))]/30 border border-dashed border-[rgb(var(--color-border-secondary))] flex items-center justify-between opacity-40 hover:opacity-100 transition-all duration-500">
                     <div className="flex items-center gap-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Simulation Environment</span>
                     </div>
                     <div className="flex gap-3">
                         <button onClick={handleDevMockEvaluation} className="px-5 py-2.5 rounded-xl bg-blue-500/10 text-blue-400 text-[10px] font-black border border-blue-500/20 hover:bg-blue-500 hover:text-white transition-all shadow-xl">Mock Analytics</button>
                         <button onClick={handleDevMockImprovement} className="px-5 py-2.5 rounded-xl bg-purple-500/10 text-purple-400 text-[10px] font-black border border-blue-500/20 hover:bg-purple-500 hover:text-white transition-all shadow-xl">Mock Upgrade</button>
                     </div>
                 </div>
            )}
        </div>
