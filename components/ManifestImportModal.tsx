...
import { 
    CheckSquare, Square, Download, FileJson, Sparkles, Loader2, AlertCircle,
    Cpu, Beaker, BookOpen, Globe, Calculator, Palette, Activity, Layers, ChevronDown, Lock, Search, X,
    Library, Check, Filter, Database
} from 'lucide-react';
...
const SubjectColors: Record<string, string> = {
    'Science': 'text-emerald-400 light:text-emerald-700 bg-emerald-500/10 light:bg-emerald-100 border-emerald-500/20 light:border-emerald-200',
    ...
    'English': 'text-purple-400 light:text-purple-700 bg-purple-500/10 light:bg-purple-100 border-purple-500/20 light:border-purple-200',
    'Mathematics': 'text-sky-400 light:text-sky-700 bg-sky-500/10 light:bg-sky-100 border-sky-500/20 light:border-sky-200',
    'Creative Arts': 'text-pink-400 light:text-pink-700 bg-pink-500/10 light:bg-pink-100 border-pink-500/20 light:border-pink-200',
    'PDHPE': 'text-red-400 light:text-red-700 bg-red-500/10 light:bg-red-100 border-red-500/20 light:border-red-200',
    'Other': 'text-gray-400 light:text-slate-600 bg-gray-500/10 light:bg-slate-100 border-gray-500/20 light:border-slate-200'
};
...
const ManifestImportModal: React.FC<ManifestImportModalProps> = ({
  ...
  const handleSubjectChange = (docId: string, newSubject: string) => {
      setLocalDocs(prev => prev.map(doc => 
          doc.id === docId ? { ...doc, subject: newSubject } : doc
      ));
  };
  ...
  const filteredGroupedDocs = useMemo(() => {
      ...
      AvailableSubjects.forEach(s => {
          const subjectDocs = localDocs.filter(doc => {
              ...
              const matchesSubject = docSubject === s || (s === 'Other' && !AvailableSubjects.includes(docSubject));
              const matchesQuery = doc.name.toLowerCase().includes(query) || doc.source.toLowerCase().includes(query);
  ...
  const hasResults = useMemo(() => 
    ...
    (Object.values(filteredGroupedDocs.groups) as { placeholders: string[] }[]).some(g => g.placeholders.length > 0),
  [filteredGroupedDocs]);
            ...
            <div className="flex items-center gap-8">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <span className="text-[10px] font-bold uppercase tracking-[0.5em] text-indigo-400 opacity-80">Content Library</span>
                        <div className="h-px w-8 bg-indigo-500/30" />
                    </div>
                    <h2 className="text-4xl font-black text-white light:text-slate-900 tracking-tight leading-none">Content Library</h2>
                    <p className="text-slate-400 light:text-slate-500 text-sm font-medium mt-3 max-w-lg leading-relaxed">
                        Synthesise your workspace with specialized NESA syllabus models. Select your core curriculum units below.
                    </p>
                </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-12 pb-12 custom-scrollbar bg-black/10 light:bg-slate-50/50 relative">
            <div className="sticky top-0 z-20 py-6 flex justify-between items-center bg-[rgb(var(--color-bg-surface))]/60 backdrop-blur-md -mx-12 px-12 border-b border-white/5 mb-8">
                <div className="flex items-center gap-6">
                    ...
                    <span className="text-xs font-bold text-slate-500">
                        Showing <span className="text-white light:text-slate-900">{filteredGroupedDocs.allVisibleDocIds.length}</span> units
                    </span>
                </div>
                ...
                >
                    {filteredGroupedDocs.allVisibleDocIds.every(id => selectedIds.has(id)) ? 'Deselect Collection' : 'Select All Visible'}
                </button>
            </div>
            ...
            {!hasResults ? (
                <div className="flex flex-col items-center justify-center py-32 animate-fade-in">
                    ...
                    <p className="text-xl font-black text-slate-500 tracking-tight">No matches found in standard registry</p>
                    <button onClick={() => setSearchQuery('')} className="mt-6 text-sm font-bold text-indigo-400 hover:underline">Reset filters</button>
                </div>
            ) : (
                        ...
                        return (
                            <section key={subject} className="animate-fade-in">
                                <div className="flex items-center gap-5 mb-8 px-2">
                                    <div className={`p-2.5 rounded-[14px] ${colorClass.split(' ')[1]} ${colorClass.split(' ')[2]} border ${colorClass.split(' ')[3]} shadow-lg`}>
                                        <Icon className={`w-5 h-5 ${colorClass.split(' ')[0]}`} />
                                    </div>
                                    <div className="flex-1">
                                        <h3 className="text-xl font-black text-white light:text-slate-900 uppercase italic tracking-tight">{subject}</h3>
                                    ...
                                    {docs.map(doc => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {isSelected && <div className="absolute -top-10 -right-10 w-24 h-24 bg-indigo-500/20 blur-3xl animate-pulse" />}
                                                <MeshOverlay opacity={isSelected ? "opacity-[0.06]" : "opacity-[0.02]"} />

                                                <div className="flex justify-between items-start mb-6 relative z-10">
                                                     <div className={`p-2 rounded-xl bg-black/20 border border-white/5 ${isSelected ? 'text-indigo-400' : 'text-slate-600'}`}>
                                                ...
                                                <div className="relative z-10 flex-1">
                                                    <h4 className={`text-lg font-black leading-tight tracking-tight mb-3 ${isSelected ? 'text-white' : 'text-slate-300 light:text-slate-800'}`}>
                                                        {doc.name}
                                                    </h4>
                                                ...
                                                <div className={`mt-6 pt-5 border-t border-white/5 relative z-10 ${isSelected ? 'block' : 'opacity-0 group-hover:opacity-100 transition-opacity duration-500'}`}>
                                    ...
                                    {placeholders.map((ph, i) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className="text-sm font-black text-slate-500 uppercase tracking-tight italic">{ph}</span>
                                            </div>
                                            <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400/50 bg-indigo-500/5 px-2.5 py-1 rounded-full border border-indigo-500/10">Coming Soon</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
