...
const TopicImportModal: React.FC<TopicImportModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-[rgb(var(--color-bg-surface))] rounded-2xl shadow-2xl w-full max-w-2xl border border-[rgb(var(--color-border-secondary))] clip-stable animate-fade-in-up overflow-hidden flex flex-col max-h-[90vh]"
        ...
        <div className="flex-grow p-6 flex flex-col overflow-hidden">
          {step === 'upload' && (
            <div className="space-y-4">
              ...
              {fileName && (
                <p className="text-center text-sm text-gray-400 mt-2">Selected file: {fileName}</p>
              )}
              {error && (
                <p className="text-center text-sm text-red-400 mt-2 bg-red-900/20 p-3 rounded-lg">
                  {error}
                </p>
              )}
            </div>
          ...
          {step === 'preview' ? (
            <>
              ...
              <button
                ...
                className="py-2.5 px-5 rounded-lg text-white font-semibold bg-gradient-to-r from-[rgb(var(--color-accent-dark))] to-[rgb(var(--color-accent))] hover:shadow-lg transition"
              >
