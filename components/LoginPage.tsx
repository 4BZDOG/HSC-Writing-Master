...
import { Lock, User as UserIcon, ArrowRight, Sparkles, BookOpen, AlertCircle, Loader2, ShieldCheck } from 'lucide-react';
...
const InputField = ({ 
  ...
  <div className="space-y-2.5">
    <label htmlFor={id} className="block text-[11px] font-bold text-slate-400 light:text-slate-600 uppercase tracking-widest ml-1">
      {label}
    </label>
    <div className={`
    ...
    `}>
      <Icon className={`ml-4 h-4 w-4 transition-colors duration-300 ${hasError ? 'text-red-400' : 'text-slate-500 group-focus-within/input:text-indigo-400'}`} />
      <input
        id={id}
        type={type}
        value={value}
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let isValid = true;
    if (!trimmedUsername) { setUsernameError(true); isValid = false; }
    if (!trimmedPassword) { setPasswordError(true); isValid = false; }
    if (!isValid) { setError('Required fields missing.'); return; }
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center relative overflow-hidden px-6 selection:bg-indigo-500/30">
      ...
      <div className="text-center mb-12 relative z-10 animate-fade-in">
        ...
        <div className="flex flex-col gap-2">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-400 opacity-80">AI Marking Assistant</span>
      ...
      {/* Main Login Card */}
      <div className="w-full max-w-[420px] relative z-10 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
        <div className="bg-[rgb(var(--color-bg-surface))] light:bg-white border-2 border-white/20 light:border-slate-200 rounded-[44px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] light:shadow-2xl overflow-hidden relative">
          ...
          <div className="p-10 relative z-10">
            <form onSubmit={handleSubmit} className="space-y-7">
              ...
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Sign In <ArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform duration-300" /></>}
              </button>
            </form>
          ...
          <div className="bg-black/40 light:bg-slate-100 px-10 py-5 border-t border-white/10 light:border-slate-200 flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-widest">
            <div className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500/50" /> Secure System</div>
        ...
        <div className="mt-10 text-center animate-fade-in" style={{ animationDelay: '500ms' }}>
             <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">Demo Accounts</p>
             <div className="flex justify-center gap-10">
                <div className="flex flex-col items-center">
                    <span className="text-white light:text-slate-800 text-xs font-mono font-bold tracking-tight px-3 py-1 rounded-lg bg-white/5 light:bg-slate-200 border border-white/10 light:border-slate-300 shadow-lg">admin</span>
                    <span className="text-[10px] text-slate-500 uppercase mt-2 font-bold tracking-wider">Admin</span>
                </div>
                <div className="flex flex-col items-center">
                    <span className="text-white light:text-slate-800 text-xs font-mono font-bold tracking-tight px-3 py-1 rounded-lg bg-white/5 light:bg-slate-200 border border-white/10 light:border-slate-300 shadow-lg">user</span>
                    <span className="text-[10px] text-slate-500 uppercase mt-2 font-bold tracking-wider">Student</span>
                </div>
             </div>
        </div>
      </div>
