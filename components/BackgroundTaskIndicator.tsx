    ...
    switch (task.status) {
      case 'running':
        return {
          ...
          progressBar: 'bg-gradient-to-r from-[rgb(var(--color-primary))] to-[rgb(var(--color-accent))]',
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0 flex-1">
            <div className="flex justify-between items-start">
                <h4 className={`text-sm font-bold ${config.textColor} truncate leading-tight`}>{task.name}</h4>
                <span className="text-[10px] font-mono text-[rgb(var(--color-text-muted))] ml-2">{Math.round(task.progress)}%</span>
            </div>
            <p className="text-xs text-[rgb(var(--color-text-muted))] truncate mt-0.5">{task.message}</p>
            {task.status === 'error' && task.error && (
                <p className="text-[10px] text-red-400 mt-1 leading-tight break-words bg-red-900/20 p-1.5 rounded border border-red-500/20">{task.error}</p>
            )}
        </div>
      </div>
