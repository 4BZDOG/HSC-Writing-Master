...
const AnswerMetricsDisplay: React.FC<AnswerMetricsDisplayProps> = ({
  ...
  return (
    <div className={`flex items-center flex-wrap gap-2 ${className}`}>
      <span
        className={`text-[10px] font-bold ${theme.text} ${theme.bg} border ${theme.border} px-2.5 py-1 rounded-lg font-mono flex items-center justify-center gap-1.5 text-center shadow-sm transition-colors duration-300`}
      >
      ...
      {keywordStats.total > 0 && (
        <span
          className={`text-[10px] font-bold ${theme.text} ${theme.bg} border ${theme.border} px-2.5 py-1 rounded-lg flex items-center justify-center gap-1.5 text-center shadow-sm transition-colors duration-300`}
        >
          <Target className={`w-3 h-3 ${theme.icon}`} />
          <span className="font-mono">
            {keywordStats.found}/{keywordStats.total} keywords
          </span>
          {showLabel && (
            <span className="hidden sm:inline opacity-70 font-sans ml-1">
              ({colourStage.label})
            </span>
          )}
        </span>
