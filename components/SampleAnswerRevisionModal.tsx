...
const SampleAnswerRevisionModal: React.FC<SampleAnswerRevisionModalProps> = ({
  ...
  const availableMarks = useMemo(
    () =>
      Array.from({ length: prompt.totalMarks + 1 }, (_, i) => i).filter(
        (m) => m !== sampleToRevise.mark && !existingMarks.includes(m)
      ),
  ...
  const targetBand = useMemo(
    () => getBandForMark(targetMark, prompt.totalMarks, commandTermInfo.tier),
    [targetMark, prompt.totalMarks, commandTermInfo.tier]
  );

  // Calculate potential band (uncapped) to detect if capping is active
  const potentialBand = useMemo(
    () => getBandForMark(targetMark, prompt.totalMarks, 6),
    [targetMark, prompt.totalMarks]
  );
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Revise sample answer"
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className={`text-xl font-bold ${bandConfig.text}`}>Revise Sample Answer</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col">
              <label
                htmlFor="mark-select"
                className="block text-sm font-semibold text-[rgb(var(--color-text-primary))] mb-3"
              >
                New Target Mark
              </label>
              <select
              ...
              <p className="text-xs text-[rgb(var(--color-text-muted))] leading-relaxed">
                The AI will adjust the quality and detail of the original answer to fit the new
                target mark. Content depth, keyword usage, and example quality will be automatically
                scaled.
              </p>
            </div>
          </div>
