    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleClose}>
      ...
      >
        <div className="relative px-6 py-5 border-b border-[rgb(var(--color-border-secondary))] light:border-slate-200 bg-[rgb(var(--color-bg-surface))] light:bg-slate-50/50">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-[rgb(var(--color-text-primary))] light:text-slate-900">Generate Syllabus Dot Points</h2>
                <p className="text-sm text-[rgb(var(--color-text-muted))] light:text-slate-500">For sub-topic: <span className="font-semibold text-[rgb(var(--color-accent))]">{subTopicName}</span></p>
              </div>
            </div>
        ...
        <div className="p-8 text-center bg-[rgb(var(--color-bg-surface))] light:bg-white">
            <p className="text-[rgb(var(--color-text-secondary))] light:text-slate-600">Let AI generate the official syllabus dot points for this sub-topic.</p>
            {error && !isLoading && <p className="text-red-400 light:text-red-600 mt-4 text-sm bg-red-900/30 light:bg-red-50 p-3 rounded-md border border-red-500/20 light:border-red-200">{error}</p>}
        </div>
