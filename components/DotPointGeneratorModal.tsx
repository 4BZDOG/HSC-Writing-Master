...
const DotPointGeneratorModal: React.FC<DotPointGeneratorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleClose}>
      <div 
        className="bg-[rgb(var(--color-bg-surface))] rounded-2xl shadow-2xl w-full max-w-xl border border-[rgb(var(--color-border-secondary))] animate-fade-in-up overflow-hidden relative" 
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-[rgb(var(--color-border-secondary))]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-[rgb(var(--color-text-primary))]">Generate Syllabus Dot Points</h2>
                <p className="text-sm text-[rgb(var(--color-text-muted))]">For sub-topic: <span className="font-semibold text-accent-light">{subTopicName}</span></p>
              </div>
            </div>
        ...
        <div className="p-8 text-center">
            <p className="text-gray-300">Let AI generate the official syllabus dot points for this sub-topic.</p>
            {error && !isLoading && <p className="text-red-400 mt-4 text-sm bg-red-900/30 p-3 rounded-md">{error}</p>}
        </div>
