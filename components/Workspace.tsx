...
import { Course, StatePath, EvaluationResult, Prompt, UserRole, Topic, SubTopic, DotPoint } from '../types';
...
const Workspace: React.FC<WorkspaceProps> = ({
  ...
  const handleSaveDraft = () => {
      ...
      if (userAnswer !== currentPrompt.userDraft) {
          syllabusHandlers.updateCourses((draft: any) => {
            findAndUpdateItem(draft, statePath, (p: any) => { p.userDraft = userAnswer; });
          });
      }
  };
  ...
  const handleSuggestOutcomes = async () => {
      ...
      try {
          const outcomes = await geminiHandlers.suggestOutcomesForPrompt(currentPrompt.question, currentCourse.outcomes, currentPrompt.totalMarks);
          if (outcomes) {
              syllabusHandlers.updateCourses((draft: any) => {
                  findAndUpdateItem(draft, statePath, (p: any) => { p.linkedOutcomes = outcomes; });
              });
          }
      } finally {
  ...
  const onInternalEvaluate = () => { handleSaveDraft(); handleEvaluate(); };

  useKeyboardShortcuts({
    'Enter': (e: KeyboardEvent) => { if (!isEvaluating && userAnswer.trim() && (e.ctrlKey || e.metaKey)) onInternalEvaluate(); },
    'F': () => onToggleFocusMode()
  });
  ...
  const handleRunQualityCheck = (content: string, type: 'question' | 'code') => {
      modalHandlers.showQualityCheck({
          ...
          onUpdate: (newContent: string) => {
             syllabusHandlers.updateCourses((draft: any) => {
                findAndUpdateItem(draft, statePath, (p: any) => { if (type === 'question') p.question = newContent; });
             });
          }
      });
  };

  useEffect(() => { setUserAnswer(currentPrompt?.userDraft || ''); }, [currentPrompt?.id, setUserAnswer]);

  const handleDevMockEvaluation = () => {
      const mockResult: EvaluationResult = {
          overallBand: 4,
          overallMark: Math.floor(currentPrompt ? currentPrompt.totalMarks * 0.7 : 10),
          overallFeedback: "Simulated evaluation for testing purposes. Shows strong grasp of theory with minor gaps in scenario application.",
          criteria: [{ criterion: "Concept Clarity", mark: 3, maxMark: 4, feedback: "Sound understanding." }],
  ...
  return (
    <div className="flex flex-col h-full gap-4">
        ...
        <div className={`grid grid-cols-1 ${isFocusMode ? 'w-full' : 'lg:grid-cols-12'} gap-6 flex-1 min-h-0 transition-all duration-500`}>
            {!isFocusMode && (
                <div className="lg:col-span-5 flex flex-col h-full overflow-y-auto pb-20 pt-0 scrollbar-hide gap-6">
                    <div className="w-full flex-shrink-0">
                        <PromptDisplay 
                            prompt={currentPrompt} isEnriching={isEnriching} enrichError={enrichError}
                            ...
                            courseOutcomes={courseOutcomes} onOutcomeClick={() => {}}
                            userRole={userRole} onDismissEnrichError={() => geminiHandlers.setEnrichError(null)}
                            onRunQualityCheck={handleRunQualityCheck}
                            onSuggestOutcomes={handleSuggestOutcomes} isSuggestingOutcomes={isSuggestingOutcomes}
                            fontSize={promptFontSize} onFontSizeChange={setPromptFontSize}
                    ...
                    <div className="border border-white/10 rounded-3xl overflow-hidden flex-shrink-0">
                        <ReferenceMaterials 
                            prompt={currentPrompt} topic={currentTopic} userRole={userRole}
                            onKeywordsChange={(kw) => syllabusHandlers.updateCourses(d => findAndUpdateItem(d, statePath, p => p.keywords = kw))}
                            onMarkingCriteriaChange={(mc) => syllabusHandlers.updateCourses(d => findAndUpdateItem(d, statePath, p => p.markingCriteria = mc))}
                            isEnriching={isEnriching} onRegenerateKeywords={geminiHandlers.handleRegenerateKeywords}
                            isRegeneratingKeywords={geminiHandlers.isRegeneratingKeywords} regenerateError={geminiHandlers.regenerateKeywordsError}
                            onSuggestKeywords={geminiHandlers.handleSuggestKeywords} isSuggestingKeywords={geminiHandlers.isSuggestingKeywords}
                            suggestError={geminiHandlers.suggestKeywordsError} userAnswer={userAnswer}
                            onAddWord={(word) => window.dispatchEvent(new CustomEvent('insert-text', { detail: word }))}
            ...
            {isFocusMode && (
                <div className="animate-fade-in max-w-5xl mx-auto w-full mb-4">
                    <PromptDisplay 
                        prompt={currentPrompt} isEnriching={isEnriching} enrichError={enrichError}
                        onVerbClick={() => setIsGuideOpen(true)} onGenerateScenario={geminiHandlers.handleGenerateScenario}
                        onUpdatePrompt={(updates) => syllabusHandlers.updateCourses(d => findAndUpdateItem(d, statePath, p => Object.assign(p, updates)))}
                        isGeneratingScenario={geminiHandlers.isGeneratingScenario} generateScenarioError={geminiHandlers.generateScenarioError}
                        courseOutcomes={courseOutcomes} onOutcomeClick={() => {}} userRole={userRole}
            ...
            <WorkspaceRightPanel 
                ...
                userAnswer={userAnswer} setUserAnswer={setUserAnswer} debouncedUserAnswer={debouncedUserAnswer}
                currentPrompt={currentPrompt} isEvaluating={isEvaluating} evaluationResult={evaluationResult}
                evaluationError={evaluationError} onEvaluate={onInternalEvaluate} onSaveDraft={handleSaveDraft}
                isImproving={isImproving} improveAnswerError={improveAnswerError} evaluatedAnswer={evaluatedAnswer}
                geminiHandlers={geminiHandlers} syllabusHandlers={syllabusHandlers} statePath={statePath}
                userRole={userRole} breadcrumbItems={breadcrumbItems} handleRunQualityCheck={handleRunQualityCheck}
                onToggleFocusMode={onToggleFocusMode} handleDevMockEvaluation={handleDevMockEvaluation}
            ...
            <CommandTermGuideModal isOpen={isGuideOpen} onClose={() => setIsGuideOpen(false)} termInfo={getCommandTermInfo(currentPrompt.verb)} />
        </div>
    </div>
  );
};
