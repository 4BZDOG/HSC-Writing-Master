    ...
    <div className="mt-3 p-4 bg-[rgb(var(--color-bg-surface-inset))] rounded-xl border border-[rgb(var(--color-border-secondary))] animate-fade-in">
      ...
      <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar pr-2">
        {data.map((course) => (
          <div
            key={course.id}
            className="bg-[rgb(var(--color-bg-surface))] p-3 rounded-lg border border-[rgb(var(--color-border-secondary))]/50"
          >
            ...
            <div className="pl-6">
              {course.topics.map((topic) => (
                <div
                  key={topic.id}
                  className="flex items-center gap-2 text-xs text-[rgb(var(--color-text-secondary))] py-0.5"
                >
              ...
              {course.topics.length === 0 && (
                <span className="text-xs text-[rgb(var(--color-text-dim))] italic">No topics</span>
              )}
            </div>
          </div>
        ))}
        {data.length === 0 && (
          <p className="text-sm text-[rgb(var(--color-text-muted))] italic">Empty backup.</p>
        )}
      </div>
    </div>
  );
      ...
      >
                ...
                {isLoading && !stats ? (
                  ...
                  <div className="space-y-8 animate-fade-in">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div
                        className={`p-5 rounded-xl border ${stats.isConnected ? 'border-emerald-500/30 bg-emerald-500/10 light:bg-emerald-50 light:border-emerald-200' : 'border-red-500/30 bg-red-500/10 light:bg-red-50'} flex flex-col`}
                      >
                        <div className="flex items-center gap-2 mb-2">
                          {stats.isConnected ? (
                            <CheckCircle className="w-5 h-5 text-emerald-400 light:text-emerald-600" />
                          ) : (
                            <AlertTriangle className="w-5 h-5 text-red-400" />
                          )}
                          <span
                            className={`text-sm font-bold ${stats.isConnected ? 'text-emerald-400 light:text-emerald-700' : 'text-red-400'}`}
                          >
                            Connection
                          </span>
                        </div>
                        <p className="text-2xl font-black text-[rgb(var(--color-text-primary))] light:text-slate-900">
                          {stats.isConnected ? 'Active' : 'Offline'}
                        </p>
                      ...
                      <div className="p-5 rounded-xl border border-[rgb(var(--color-border-secondary))] light:border-slate-200 bg-[rgb(var(--color-bg-surface-inset))]/30 light:bg-slate-50 flex flex-col">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-bold text-blue-400 light:text-blue-700">
                            Storage
                          </span>
                        </div>
                        <p className="text-2xl font-black text-[rgb(var(--color-text-primary))] light:text-slate-900">
                        ...
                        <p className="text-xs text-[rgb(var(--color-text-muted))] light:text-slate-500 mt-1">
                          of {stats.quota ? formatBytes(stats.quota.quota) : 'Unknown'} Available
                        </p>
                      </div>
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-[rgb(var(--color-text-primary))] light:text-slate-900 mb-4 flex items-center gap-2">
                        <Archive className="w-5 h-5 text-[rgb(var(--color-accent))]" /> Object
                        Stores
                      </h3>
                        ...
                        <table className="w-full text-left text-sm">
                          ...
                          <tbody className="divide-y divide-[rgb(var(--color-border-secondary))]/30 light:divide-slate-200">
                            {stats.stores.map((store) => (
                              <tr
                                key={store.name}
                                className="hover:bg-[rgb(var(--color-bg-surface-light))]/20 light:hover:bg-slate-100 transition-colors"
                              >
                                <td className="px-6 py-4 font-mono text-[rgb(var(--color-text-primary))] light:text-slate-800">
                                  {store.name}
                                </td>
                                <td className="px-6 py-4 text-[rgb(var(--color-text-secondary))] light:text-slate-600">
                                  {store.count}
                                </td>
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                {isLoadingBackups ? (
                  ...
                  <div className="text-center py-12 bg-[rgb(var(--color-bg-surface-inset))]/20 light:bg-slate-50 rounded-xl border border-dashed border-[rgb(var(--color-border-secondary))] light:border-slate-300">
                    ...
                    <p className="text-xs text-[rgb(var(--color-text-muted))] light:text-slate-500">
                      Backups will appear here as you work.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {backups.map((backup) => (
                      <div key={backup.key}>
                        ...
                        {previewId === backup.key && <SnapshotPreview data={previewData} />}
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="flex-1 rounded-xl border border-[rgb(var(--color-border-secondary))] light:border-slate-300 bg-[#0d1117] light:bg-slate-50 p-4 overflow-auto custom-scrollbar relative shadow-inner">
                  {isLoadingData ? (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <LoadingIndicator
                        messages={['Fetching records...', 'Formatting JSON...']}
                        duration={1}
                      />
                    </div>
                  ) : (
        ...
        <div className="px-6 py-5 bg-[rgb(var(--color-bg-surface-inset))]/30 light:bg-slate-50 border-t border-[rgb(var(--color-border-secondary))] light:border-slate-200 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-lg text-sm font-medium text-[rgb(var(--color-text-muted))] light:text-slate-600 hover:text-white light:hover:text-slate-900 transition hover:bg-[rgb(var(--color-bg-surface-light))] light:hover:bg-slate-200"
            >
              Close
            </button>
          </div>
