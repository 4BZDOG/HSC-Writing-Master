...
const Breadcrumb: React.FC<BreadcrumbProps> = ({ items }) => {
  ...
  const getLevelIcon = (index: number) => {
      switch (index) {
          case 0: return <BookOpen className="w-4 h-4" />; // Course
          case 1: return <Layers className="w-4 h-4" />;   // Topic
          case 2: return <Folder className="w-4 h-4" />;   // SubTopic
          case 3: return <Hash className="w-4 h-4" />;     // Dot Point
          default: return <FileText className="w-4 h-4" />; // Question
      }
  };
          ...
          return (
            <li key={`breadcrumb-${index}`} className="flex items-center flex-shrink-0">
              ...
              <button
                ...
                `}
              >
                <span className={`opacity-70 ${isLast ? 'text-[rgb(var(--color-accent))] opacity-100' : ''}`}>
                    {getLevelIcon(index)}
                </span>
