...
const itemTypeStyles: { [key: string]: { icon: React.ReactNode, color: string } } = {
  course: { icon: <BookOpen className="w-3.5 h-3.5" />, color: 'text-[rgb(var(--color-accent))] light:text-sky-600' },
  topic: { icon: <Folder className="w-3.5 h-3.5" />, color: 'text-purple-400 light:text-purple-600' },
  subTopic: { icon: <Folder className="w-3.5 h-3.5" />, color: 'text-indigo-400 light:text-indigo-600' },
  dotPoint: { icon: <Hash className="w-3.5 h-3.5" />, color: 'text-[rgb(var(--color-text-secondary))] light:text-slate-500' },
};
...
const TreeItemComponent: React.FC<TreeItemComponentProps> = ({ item, selectedIds, expandedIds, onToggleSelect, onToggleExpand, level }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className={`flex-grow text-xs truncate select-none ${isSelected ? 'font-semibold text-[rgb(var(--color-text-primary))] light:text-slate-900' : 'text-[rgb(var(--color-text-secondary))] light:text-slate-700'}`}>
            {item.label}
        </span>
      </div>
