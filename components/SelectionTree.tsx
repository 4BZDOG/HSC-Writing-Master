...
const itemTypeStyles: { [key: string]: { icon: React.ReactNode, color: string } } = {
  ...
  dotPoint: { icon: <Hash className="w-3.5 h-3.5" />, color: 'text-[rgb(var(--color-text-secondary))]' },
};
...
const TreeItemComponent: React.FC<TreeItemComponentProps> = ({ item, selectedIds, expandedIds, onToggleSelect, onToggleExpand, level }) => {
  ...
  return (
    <div className="relative">
       {level > 0 && <div className="absolute left-0 top-0 bottom-0 w-px bg-[rgb(var(--color-border-secondary))]/30 ml-[11px]" style={{ left: `${(level-1) * 20}px` }}></div>}
      ...
      >
        ...
        <span className={`flex-grow text-xs truncate select-none ${isSelected ? 'font-semibold text-[rgb(var(--color-text-primary))]' : 'text-[rgb(var(--color-text-secondary))]'}`}>
            {item.label}
        </span>
      </div>
