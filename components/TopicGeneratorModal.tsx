    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleClose}>
      ...
      >
        <div className="relative px-6 py-5 border-b border-[rgb(var(--color-border-secondary))] light:border-slate-200 overflow-hidden bg-[rgb(var(--color-bg-surface))] light:bg-slate-50/50">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-[rgb(var(--color-text-primary))] light:text-slate-900">Suggest a Topic</h2>
                <p className="text-sm text-[rgb(var(--color-text-muted))] light:text-slate-500">Let AI find a new topic for "{courseName}".</p>
              </div>
            </div>
            <button onClick={handleClose} className="w-9 h-9 rounded-lg bg-[rgb(var(--color-bg-surface-inset))]/50 light:bg-slate-200 hover:bg-[rgb(var(--color-border-secondary))] light:hover:bg-slate-300 transition-all duration-200 flex items-center justify-center group">
        ...
        <div className="p-6 space-y-4 bg-[rgb(var(--color-bg-surface))] light:bg-white">
          {!isLoading && suggestedTopic && (
            <div className="bg-[rgb(var(--color-bg-surface-inset))]/50 light:bg-amber-50 p-6 rounded-xl border border-[rgb(var(--color-border-secondary))] light:border-amber-100 flex flex-col items-center text-center gap-2">
              <p className="text-gray-400 light:text-amber-800/60 text-xs font-bold uppercase tracking-widest">AI Suggestion</p>
              <p className="text-xl font-bold text-[rgb(var(--color-text-primary))] light:text-amber-900 leading-tight">{suggestedTopic}</p>
            </div>
          )}
          {error && !isLoading && (
            <div className="text-red-400 light:text-red-600 bg-red-900/30 light:bg-red-50 border border-red-500/20 light:border-red-200 p-3 rounded-md text-sm">{error}</div>
          )}
        </div>
        ...
        <div className="px-6 py-5 border-t border-[rgb(var(--color-border-secondary))] light:border-slate-200 bg-[rgb(var(--color-bg-surface-inset))]/30 light:bg-slate-50 flex items-center justify-end">
          <div className="flex items-center gap-3">
            <button onClick={handleClose} className="py-2.5 px-5 rounded-lg font-medium text-[rgb(var(--color-text-muted))] light:text-slate-600 bg-[rgb(var(--color-bg-surface-inset))]/50 light:bg-white hover:bg-[rgb(var(--color-border-secondary))] light:hover:bg-slate-100 border border-transparent light:border-slate-200 transition-all duration-200">
              Cancel
            </button>
            <button onClick={handleGenerate} disabled={isLoading} className="py-2.5 px-5 rounded-lg font-semibold text-white bg-[rgb(var(--color-bg-surface-light))] hover:bg-[rgb(var(--color-border-secondary))] light:bg-slate-400 light:hover:bg-slate-500 transition-all duration-200 disabled:opacity-50 flex items-center gap-2">
        ...
        {isLoading && (
          <div className="absolute inset-0 bg-[rgb(var(--color-bg-surface))]/95 light:bg-white/95 backdrop-blur-sm flex items-center justify-center">
            <div className="w-full max-w-md mx-6">
              <LoadingIndicator messages={['Analysing syllabus...', 'Finding an uncovered topic...']} duration={8} band={3} />
            </div>
          </div>
        )}
      </div>
